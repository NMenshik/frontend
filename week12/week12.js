...
const makeSixteen = () => {
	const str = 'JavaScript';
	resultSixteen.textContent = str.substr(3); //по материалу этот метод подходит лучше, если не знаешь длину строки, то есть последний индекс
};
...
const makeTwentyOne = () => {
	resultTwentyOne.textContent = Math.round(Math.random() * 10); //вот здесь не знаю, в документации не очень понятно как это работает
};
